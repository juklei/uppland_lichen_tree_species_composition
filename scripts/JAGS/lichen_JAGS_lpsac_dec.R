## lichen lpsac model with % deciduous as the covariates
##
## First edit: 20190605
## Last edit: 20200710
##
...
model{
  ...
  for(p in 1:nplot){
    ...
    mu_rich[p] <- r_alpha +
                  r_beta_dec*dec[p,1] + r_beta2_dec*dec[p,1]^2 +
                  r_beta_dbh*dbh[p,1]
    ## Saturation:
    ...
    log(mu_sat[p]) <- s_alpha +
                      s_beta_dec*dec[p,1] + s_beta2_dec*dec[p,1]^2 +
                      s_beta_dbh*dbh[p,1]
  }
  ...
  for(m in 1:length(dec_pred)){
    r_dec[m] <- r_alpha + r_beta_dec*dec_pred[m] + r_beta2_dec*dec_pred[m]^2
    log(s_dec[m]) <- s_alpha + s_beta_dec*dec_pred[m] + s_beta2_dec*dec_pred[m]^2
  }
