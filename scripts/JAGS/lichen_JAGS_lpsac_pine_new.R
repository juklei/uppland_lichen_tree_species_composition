...
model{
  ...
  for(p in 1:nplot){
    for(k in 1:nrep){
      ...
      log(lambda_rich[k,p]) <- r_alpha + r_ek[k] +
                               r_beta_pine*pine[p,1] + 
                               r_beta2_pine*pine[p,1]^2 +
                               r_beta_dbh*dbh[p,1]
      ## Saturation:
      ...
      log(mu_sat[k,p]) <- s_alpha + s_ek[k] +
                          s_beta_pine*pine[p,1] + 
                          s_beta2_pine*pine[p,1]^2 +
                          s_beta_dbh*dbh[p,1]
  }}
  ...
  for(m in 1:length(pine_pred)){
    log(r_pine[m]) <- r_alpha + 
                      r_beta_pine*pine_pred[m] + 
                      r_beta2_pine*pine_pred[m]^2
    log(s_pine[m]) <- s_alpha +
                      s_beta_pine*pine_pred[m] +
                      s_beta2_pine*pine_pred[m]^2
  }
